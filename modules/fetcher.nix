{ pkgs, lib, config, prefix, ... }: with lib; with config.version; {
	options.version = {
		branch = mkOption {
		...
		buildId = mkOption {
			...
			type = types.ints.positive;
				...
				default = throw "nekocord.buildId must be expliclty defined. The latest buildId on the ${branch} branch is ${builtins.toString (nekocord.latest.nekocord.${branch})}.";
			};
		url = mkOption {
			description = "The URL to fetch Nekocord from. This should not need to be manually specified.";
			type = types.strMatching "https?://.+";
			default = nekocord.buildURL { inherit branch; id = buildId; };
		};
		hash = mkOption {
